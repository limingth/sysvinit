...
\subsection{分析 -o omitpid 参数创建双向链表 omit}
...
\subsection{暂时忽略 SIGTERM/SIGSTOP/SIGKILL 信号}
...
01064 
\end{verbatim}\end{shaded}}
\subsection{读 /proc 文件系统，建立进程链表 plist}
...
\subsection{根据 plist 链表开始依次 kill 进程}
...
01092         }
\end{verbatim}\end{shaded}}
\subsection{恢复所有进程运行 （从 STOP 又回到 CONT）}
...
01098         closelog();
\end{verbatim}\end{shaded}}
\subsection{调用 usleep(1) 强制 Kernel 运行调度器}
